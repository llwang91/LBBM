...
def sym_init():
    for ix in range(64):
        for iy in range(ix,64):
            for iz in range(ix,64):
                if iz<=iy:
                    xdof.append(ix)
                    ydof.append(iy)
                    zdof.append(iz)
    for iz in range(64):
        for iy in range(iz,64):
            for ix in range(iz,64):
                if ix<=iy:
                    xdof2.append(ix)
                    ydof2.append(iy)
                    zdof2.append(iz)
...
def make_sym(x,device=None):
    ...
    newx[:,:,xdof,zdof,ydof]=x[:,:,xdof,zdof,ydof]
    newx[:,:,xdof2,zdof2,ydof2]=newx[:,:,xdof,zdof,ydof]
    newx[:,:,xdof3,zdof3,ydof3]=newx[:,:,xdof,zdof,ydof]
    newx[:,:,xdof5,zdof5,ydof5]=newx[:,:,xdof4,zdof4,ydof4]
    #return (torch.transpose(torch.triu(x,diagonal=1),dim-2,dim-1)+torch.triu(x,diagonal=0)).to(device)
    return newx.to(device)
